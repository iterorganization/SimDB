...
def test_remote_watchers_list_command(list_watchers, get_api_version):
    sim_id = 'acbd1234'
    watchers = ['a', 'b', 'c']
...
def test_remote_watcher_remove_command(remove_watcher, get_api_version):
    user = 'test'
    sim_id = 'acbd1234'
    config_file = config_test_file()
    runner = CliRunner()
...
def test_remote_watcher_add_command(add_watcher, get_api_version):
    user = 'test'
    email = 'test@iter.org'
    sim_id = 'acbd1234'
    config_file = config_test_file()
...
def test_remote_list_command(list_simulations, get_api_version):
    data = [
        ('abcd1234', 'test'),
        ('abcd5678', 'test'),
        ('abcd4321', 'test'),
...
@mock.patch('simdb.cli.remote_api.RemoteAPI.get_api_version')
@mock.patch('simdb.cli.remote_api.RemoteAPI.list_simulations')
def test_remote_list_command_with_verbose(list_simulations, get_api_version):
    data = [
...
def test_remote_info_command(get_simulation, get_api_version):
    sim_id = 'abcd1234'
...
def test_remote_update_command_with_accept(validate_simulation, update_simulation, get_api_version):
