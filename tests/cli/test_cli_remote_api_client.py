...
def test_the_api_version_comes_from_the_negotiated_endpoint(http):
    http.routes[""] = response({**INDEX, "api_version": None})

    api = RemoteAPI("test", None, None, make_config())

    # The negotiated endpoint decides the version, so a remote that reports no
    # api_version of its own is still usable.
